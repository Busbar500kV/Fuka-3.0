...
class Engine:
    """
    Full-grid mode: S matches env (H×W); no resampling.
    """
    ...
    def _env_field(self, t: int) -> np.ndarray:
        # respect env_frames if provided
        t = int(t % max(1, self.env_frames))
        ...
        for src in self.env_sources:
            ...
            if kind == "moving_peak_2d":
                ...
                # speeds are fractions of grid per frame → scale like env.py
                vx  = float(src.get("speed_x", 0.0)) * W
                vy  = float(src.get("speed_y", 0.0)) * H
